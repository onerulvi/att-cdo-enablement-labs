...
4. Copy and paste this statement into the empty SQL box:
   ...
   When run, the SQL statement above selects and creates a set of columns. The columns that will be generated in this table are shown below:
...
Repeat the steps above (2-4) to add a second parameter called **End_Year** with a Default value of **2021**. It should look like the image below:
...
Here you can see the new job schedule was added, your runtime parameters that were set in DataStage, settings, and notifications. No other changes will be made at this time.
...
<details id="7-summary">
<summary><h3>7. Summary</h3></summary>

In this lab, you used DataStage to:

- Connect to a Db2 Warehouse to access employee data
- Extract data from multiple tables using SQL select statements
- Transform and join the data to create a unified employee ranking dataset
- Create parameterized queries for flexible year-based filtering
- Sort the data by employee ranking
- Output the results to a CSV file for further analysis
- Schedule the job to run automatically on a recurring basis

DataStage's ETL/ELT capabilities within watsonx.data integration provide a powerful, flexible platform for data integration tasks. The "design once, run anywhere" approach enables you to create integration flows that can be deployed across cloud and on-premises environments, ensuring continuity and flexibility for your data engineering needs.

**Key Benefits Demonstrated:**

- **Automated data integration** - Eliminates manual data preparation tasks
- **Dynamic logging** - Quickly identify and navigate to warnings/errors in job logs
- **Flexible scheduling** - Set up recurring jobs to keep data current
- **Parameterization** - Easily adjust data selection criteria at runtime
- **Visual design** - Intuitive canvas for building complex data flows

Congratulations on completing the DataStage lab!
